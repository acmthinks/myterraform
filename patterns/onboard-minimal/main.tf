...
##
## if "is_admin_service_id" is true, this modeule block will execute 
###############################################################################
module "admin_service_id" {
  count = var.is_admin_service_id ? 1 : 0
...
##
## if "is_sre_service_id" is true, this modeule block will execute 
###############################################################################
module "sre_service_id" {
  count = var.is_sre_service_id ? 1 : 0
  source = "../../modules/service-id"
...
##
## if "is_audit_service_id" is true, this modeule block will execute 
###############################################################################
module "audit_service_id" {
  count = var.is_audit_service_id ? 1 : 0
  source = "../../modules/service-id"
...
##
## if "is_users_service_id" is true, this modeule block will execute 
###############################################################################
module "users_service_id" {
  count = var.is_users_service_id ? 1 : 0
  source = "../../modules/service-id"

  region           = var.region
  ibmcloud_api_key = var.ibmcloud_api_key
  prefix           = join (" ", [var.prefix, "User"])
  roles            = ["Editor"]
  resource_group_id= module.resource_group.id
  access_group_id  = module.access_user_group.id
  description      = join(" ", ["USERS Service ID for", module.resource_group.name])
}
...
module "access_user_group" {
  ...
  prefix           = var.prefix
  group-type       = "user"
  description      = join(" ", ["User access group for", module.resource_group.name])
  members          = var.users_members
}
...
module "sandbox_vpc" {
  ...
  region           = var.region
  ibmcloud_api_key = var.ibmcloud_api_key
  basename         = join("-", [var.prefix, "sandbox"])
  resource_group_id= module.resource_group.id
}

module "sample_vsi" {
  source = "../../modules/vsi-ubuntu"

  region           = var.region
  zone             = "us-south-1"
  ibmcloud_api_key = var.ibmcloud_api_key
  basename         = join("-", [var.prefix, "sample"])
  resource_group_id= module.resource_group.id
